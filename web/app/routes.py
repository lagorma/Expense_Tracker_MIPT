...
from app import db
from app.forms import RegistrationForm, EditProfileForm, AddExpenseForm, LoginForm
...
def index():
    expenses = [
        {"category": "products", "body": "23$"},
        {"category": "clothes", "body": "100$"},
        {"category": "beauty products", "body": "58$"},
    ]
...
def user(username):
    ...
    expenses = Expense.query.filter_by(user_id=user.id)  # FIXED THIS LINE
    # removed expense() in User class
...
def add_expense():
    ...
    if form.validate_on_submit():
        expense = Expense(
            category=form.category.data, body=form.body.data, timestamp=form.timestamp.data, user_id=current_user.id
        )
